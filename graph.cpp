...
typedef struct {
    ...
    int* attribute; // the attribute dynamic array for nodes
    int attr_dimension = 0; // attribute set dimension, mainly 2D (two attr in graph)
...
specialsparse* readedgelist(char* edgelist, char* attr_file){
...
/* Read attribute File */
void read_attribute_file(specialsparse* g, const char* attr_file) {
    ...
    while (fscanf(f, "%d %d", &curr_node, &curr_attr) == 2) {
        ...
        attributes[curr_node] = curr_attr;
        if (curr_attr > g->attr_dimension) {
            g->attribute = curr_attr;
        }
    }
    attr_dimension++;
    fclose(f);
